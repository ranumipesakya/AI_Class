  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Discover Courses</h1>
          <p className="text-slate-600 mt-2">Find the right classes to boost your results.</p>
        </div>
          ...
          <div key={course.id} className="card flex flex-col cursor-pointer hover:border-primary-300">
            ...
            <div className="p-5 flex-grow flex flex-col">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">{course.language} Medium</span>
              </div>
              <h3 className="text-lg font-bold text-slate-900 mb-1 leading-snug">{course.title}</h3>
              <p className="text-sm text-slate-600 mb-4">By {course.teacher}</p>
              
              <div className="mt-auto pt-4 border-t flex justify-between items-center text-sm text-slate-500">
                <span>{course.students.toLocaleString()} students</span>
                <Link to={`/courses/${course.id}/lesson/1`} className="text-primary-600 font-semibold hover:text-primary-700">View</Link>
              </div>
            </div>
          </div>
        ))}
